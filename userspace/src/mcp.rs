...
pub fn is_mcp_jsonrpc(body: &str) -> bool {
    if !body.contains("jsonrpc") { return false; }
...
pub fn parse_jsonrpc_mcp(body: &str) -> Option<McpEvent> {
    ...
    if val.get("jsonrpc").and_then(|v| v.as_str()) != Some("2.0") { return None; }
    ...
    Some(McpEvent { method, id, tool_name, has_injection, permission_flags })
}
...
mod tests {
    ...
    fn test_is_mcp_jsonrpc() {
        assert!(is_mcp_jsonrpc(r#"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}"#));
        assert!(is_mcp_jsonrpc(r#"{"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}}"#));
        assert!(is_mcp_jsonrpc(r#"{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"read_file"}}"#));
        // Not MCP — generic JSON-RPC
        assert!(!is_mcp_jsonrpc(r#"{"jsonrpc":"2.0","id":1,"method":"eth_blockNumber","params":[]}"#));
        // Not JSON-RPC
