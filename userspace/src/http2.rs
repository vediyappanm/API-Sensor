...
/// Decoded HPACK header list: `(name, value)` byte pairs.
pub type HpackHeaders = Vec<(Vec<u8>, Vec<u8>)>;
...
impl Http2HpackDecoder {
    ...
    // The unit error is intentional: decode failures are non-actionable here —
    // callers simply skip the block — so a richer error type would add no value.
    #[allow(clippy::result_unit_err)]
    pub fn decode(&mut self, block: &[u8]) -> Result<HpackHeaders, ()> {
