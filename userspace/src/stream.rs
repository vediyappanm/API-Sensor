        ...
        for msg in msgs {
            match msg {
                ...
                HttpMessage::Response(resp) => {
                    ...
                    let is_mcp_json =
                        !is_mcp_sse && request.body.as_deref().map(is_mcp_jsonrpc).unwrap_or(false);
                    ...
                    let source = if ev.ssl_ptr & PLAINTEXT_FLAG != 0 {
                        "ebpf-plaintext"
                    } else {
                        "ebpf"
                    };
...
pub fn build_event(
    ...
    ApiTrafficEvent {
        ...
        request: ApiRequest {
            ...
            path,
            host: req.host,
            scheme: if source.starts_with("ebpf-plaintext") {
                "http".to_string()
            } else {
                "https".to_string()
            },
